...
export default function CrossComparisonTab() {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <h3 className="font-semibold text-lg mb-2" style={{ color: "#1E293B" }}>
          Cross-Comparison Not Available
        </h3>
        <p className="text-sm max-w-sm" style={{ color: "#94A3B8" }}>
          Compare reviews between (at least 2) properties.
        </p>
      </div>
    );
  }
  ...
  const radarData = ["positive", "negative", "neutral", "mixed"].map(
    (metric) => {
      const entry = {
      ...
      categories.forEach((cat) => {
        entry[cat] = Math.round(
          (sentiment_by_category[cat]?.[metric] || 0) * 100,
        );
      });
      return entry;
    },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          ...
          <div>
            ...
            <div className="text-xs mt-0.5" style={{ color: "#64748B" }}>
              {Math.round(
                (sentiment_by_category[best_category]?.positive || 0) * 100,
              )}
              % positive sentiment ·{" "}
        ...
        >
          ...
          <div>
            ...
            <div className="text-xs mt-0.5" style={{ color: "#64748B" }}>
              {Math.round(
                (sentiment_by_category[worst_category]?.negative || 0) * 100,
              )}
              % negative sentiment ·{" "}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {categories.map((cat, i) => {
          ...
          const pos = Math.round(
            (sentiment_by_category[cat]?.positive || 0) * 100,
          );
          const neg = Math.round(
            (sentiment_by_category[cat]?.negative || 0) * 100,
          );
          ...
          return (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div
                  className="flex justify-between text-xs mt-1 pt-1.5"
                  style={{ borderTop: "1px solid #E5E7EB" }}
                >
                  <span style={{ color: "#94A3B8" }}>{count} reviews</span>
                  <span
                    className="font-medium"
                    style={{ color: botRate > 20 ? "#DC2626" : "#94A3B8" }}
                  >
                    {botRate}% bots
                  </span>
                </div>
