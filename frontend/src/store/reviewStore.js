...
export const useReviewStore = create((set, get) => ({
  ...
  startDemoAnalysis: async () => {
    set({ jobStatus: 'queued', report: null, currentJob: null, progressEvents: [] })
    try {
  ...
  startPasteAnalysis: async (reviews) => {
    set({ jobStatus: 'queued', report: null, currentJob: null, progressEvents: [] })
    try {
      const { data } = await axios.post(`${API_BASE}/analyze/paste`, { reviews })
      set({ currentJob: data.job_id })
      get().startPolling(data.job_id)
      return data
    } catch (e) {
  ...
  startTextAnalysis: async (text, category, productName) => {
    ...
    try {
      const { data } = await axios.post(`${API_BASE}/analyze/text`, {
        text, category, product_name: productName
      })
      set({ currentJob: data.job_id })
      get().startPolling(data.job_id)
      return data
  ...
  downloadPDF: async () => {
    ...
    try {
      ...
      link.href = url
      link.setAttribute('download', `reviewiq_report_${currentJob}.pdf`)
  ...
  reset: () => {
    ...
    set({ currentJob: null, jobStatus: 'idle', report: null, pollingInterval: null, progressEvents: [] })
  },
  ...
  getTrendChartData: () => {
    ...
    for (const point of report.trend_data) {
      ...
      if (!byWindow[key]) {
        byWindow[key] = { window: key, windowStart: point.window_start, windowEnd: point.window_end }
      }
      byWindow[key][point.feature + '_complaints'] = Math.round(point.complaint_rate * 100)
      byWindow[key][point.feature + '_praises'] = Math.round(point.praise_rate * 100)
    }
  ...
  getBotRiskData: () => {
    ...
    return {
      riskCounts,
      topSignals: Object.entries(topSignals).sort((a, b) => b[1] - a[1]).slice(0, 8),
      authenticRate: Math.round(((report.clean_reviews || 0) / Math.max(report.total_reviews, 1)) * 100),
      botRate: Math.round(((report.bot_suspected_count || 0) / Math.max(report.total_reviews, 1)) * 100),
    }
  },
  ...
  getProductSatisfactionData: () => {
    ...
    const UNSATISFIED = new Set(['negative', 'mixed', 'sarcastic', 'ambiguous'])
    ...
    reviews.forEach(r => {
      ...
      if (!byProduct[name]) {
        byProduct[name] = { product: name, satisfied: 0, unsatisfied: 0, total: 0, sentiments: {} }
      }
    ...
    return Object.values(byProduct)
      ...
      .map(p => ({
        ...p,
        satisfiedRate:   p.total > 0 ? Math.round((p.satisfied   / p.total) * 100) : 0,
        unsatisfiedRate: p.total > 0 ? Math.round((p.unsatisfied / p.total) * 100) : 0,
      }))
  ...
  getSentimentTypeCounts: () => {
    ...
    const counts = { positive: 0, negative: 0, neutral: 0, mixed: 0, sarcastic: 0, ambiguous: 0 }
    reviews.forEach(r => {
