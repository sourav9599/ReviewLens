    ...
    >
      <div
        ...
        style={{
          ...
          background:
            score >= 0.8 ? "#28C76F" : score >= 0.5 ? "#FFB547" : "#EA5455",
        }}
      />
    </div>
...
export default function ReviewsTab() {
  ...
  const categories = [
    "all",
    ...new Set(reviews.map((r) => r.product_category).filter(Boolean)),
  ];

  const filtered = useMemo(() => {
    return reviews.filter((r) => {
      if (filterStatus !== "all" && r.status !== filterStatus) return false;
      if (filterSentiment !== "all" && r.overall_sentiment !== filterSentiment)
        return false;
      if (filterCategory !== "all" && r.product_category !== filterCategory)
        return false;
      if (search) {
        const q = search.toLowerCase();
        if (
          !r.cleaned_text?.toLowerCase().includes(q) &&
  ...
  const sentCounts = useMemo(() => {
    const c = {};
    reviews.forEach((r) => {
      c[r.overall_sentiment] = (c[r.overall_sentiment] || 0) + 1;
    });
    return c;
  }, [reviews]);
    ...
    <div className="space-y-5">
      ...
      <div className="card-3d p-4">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          >
            {[
              ...
              <option key={o} value={o}>
                {o === "all" ? "All Status" : STATUS_STYLES[o]?.label || o}
              </option>
            ))}
          </select>
          ...
          >
            {filtered.length}{" "}
            <span style={{ color: "#94A3B8" }}>of {reviews.length}</span>
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          ...
          {Object.entries(SENTIMENT_CONFIG).map(([key, cfg]) => (
            <button
              key={key}
              onClick={() => {
                setFilterSentiment(filterSentiment === key ? "all" : key);
                setPage(0);
              }}
              className="px-3 py-1 rounded-full text-xs font-semibold transition-all"
              style={{
      ...
      <div className="space-y-3">
        {paged.map((review, i) => {
          const statusStyle =
            STATUS_STYLES[review.status] || STATUS_STYLES.clean;
          const sentCfg =
            SENTIMENT_CONFIG[review.overall_sentiment] ||
              ...
              <div className="flex items-start gap-4">
                ...
                <div
                  ...
                  style={{
                    ...
                    border: `1px solid ${sentCfg.border}`,
                  }}
                >
                  {sentCfg.emoji}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    ...
                    {review.rating && (
                      <span
                        ...
                        style={{ color: "#D97706" }}
                      >
                        <Star size={10} fill="#D97706" strokeWidth={0} />{" "}
                        {review.rating}
                      </span>
                    )}
                    {/* Language */}
                    {review.flags?.includes("multilingual") && (
                      ...
                      >
                        <Globe size={9} />{" "}
                        {review.detected_languages?.join(", ")}
                      </span>
                    )}
                    {/* Date */}
                    {review.review_date && (
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {/* Bot risk */}
                    {review.bot_risk_level &&
                      review.bot_risk_level !== "low" && (
                        <span
                          className="text-xs font-semibold px-2 py-0.5 rounded-full"
                          style={{
                            background: "#FEE2E2",
                            color:
                              BOT_RISK_STYLES[review.bot_risk_level]?.color,
                          }}
                        >
