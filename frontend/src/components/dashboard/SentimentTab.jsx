  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <h4
          className="font-semibold text-xs capitalize leading-tight"
          style={{ color: "#1E293B" }}
        >
          {feature.replace(/_/g, " ")}
        </h4>
        <span
          className={`sentiment-badge ${cfg.badgeClass} flex-shrink-0 ml-1`}
        >
          {cfg.emoji} {cfg.label}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 mb-1.5">
        ...
        >
          <div
            ...
            style={{
              width: `${data.praise_rate * 100}%`,
              background: "#28C76F",
            }}
          />
        </div>
        <span className="text-xs w-10" style={{ color: "#94A3B8" }}>
          👍
        </span>
      </div>
      {/* Complaint bar */}
      ...
      <div className="mt-2 text-xs" style={{ color: "#94A3B8" }}>
        {data.total_mentions} mentions · {Math.round(data.avg_confidence * 100)}
        % confidence
      </div>
    </motion.div>
  );
...
function FeatureDetail({ feature, data }) {
  const sentimentData = [
    ...
    {
      name: "Sarcastic",
      value: data.sarcastic || 0,
      color: "#A78BFA",
      bg: "#EDE9FE",
    },
    {
      name: "Ambiguous",
      value: data.ambiguous || 0,
      color: "#60A5FA",
      bg: "#DBEAFE",
    },
  ].filter((d) => d.value > 0);
  ...
  return (
    ...
    >
      ...
      <p className="text-xs mb-5" style={{ color: "#94A3B8" }}>
        {data.total_mentions} total mentions · Avg confidence:{" "}
            ...
            <div key={d.name} className="flex items-center gap-2 mb-2.5">
              ...
              >
                <div
                  className="h-full rounded-full"
                  style={{
                    width: `${data.total_mentions ? (d.value / data.total_mentions) * 100 : 0}%`,
                    background: d.color,
                  }}
                />
              </div>
          ...
          {[
            {
              ...
              val: Math.round(data.complaint_rate * 100),
              color: "#EA5455",
              bg: "#FEE2E2",
            },
            {
              label: "Praise Rate",
              val: Math.round(data.praise_rate * 100),
              color: "#28C76F",
              bg: "#DCFCE7",
            },
            {
              label: "Avg Confidence",
              val: Math.round(data.avg_confidence * 100),
              color: "#2563EB",
              bg: "#DBEAFE",
            },
        ...
        <div className="mt-4">
          ...
          <div className="space-y-2">
            {data.representative_reviews.slice(0, 3).map((r, i) => {
              const cfg =
                SENTIMENT_CONFIG[r.sentiment] || SENTIMENT_CONFIG.neutral;
              return (
                <div
                  key={i}
                ...
                >
                  <div
                    className="font-semibold mb-0.5 capitalize flex items-center gap-1.5"
...
export default function SentimentTab() {
  const { report, getFeatureSummaryData, getSentimentTypeCounts } =
    useReviewStore();
  ...
  const selectedData = selectedFeature
    ? report.feature_summary[selectedFeature]
    : null;
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-xs mb-5" style={{ color: "#94A3B8" }}>
          Complete breakdown across all {Object.keys(SENTIMENT_CONFIG).length}{" "}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="card-3d p-6 lg:col-span-1">
          ...
          <p className="section-subtext">
            Top features by complaint & praise rate
          </p>
          ...
          <div className="flex items-center gap-4 justify-center">
            {[
              { color: "#EA5455", label: "Complaints" },
              { color: "#28C76F", label: "Praises" },
            ].map(({ color, label }) => (
      ...
      <div>
        <h3 className="section-header mb-4">
          All Features ({featureData.length})
        </h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {featureData.map(({ feature }) => (
            <FeatureCard
              ...
              feature={feature}
              data={
                report.feature_summary[feature.replace(/ /g, "_")] ||
        ...
        {[
          {
            ...
            key: "sarcastic",
            cfg: SENTIMENT_CONFIG.sarcastic,
            desc: "Flagged for human review — sentiment is likely inverted",
          },
          {
            label: "Ambiguous Reviews",
            key: "ambiguous",
            cfg: SENTIMENT_CONFIG.ambiguous,
            desc: "Unclear intent — not forced into positive/negative",
          },
          {
            label: "Multilingual Reviews",
            key: "multilingual",
            cfg: {
              color: "#0891B2",
              bg: "#ECFEFF",
            ...
            desc: "Hindi, Hinglish, or other Indian languages detected",
          },
        ].map(({ label, key, cfg, desc }) => {
          const count =
            key === "multilingual"
              ? report.processed_reviews?.filter((r) => r.flags?.includes(key))
                  .length || 0
              : sentCounts[key] || 0;
          return (
            <motion.div
