...
import {
  BarChart3, TrendingUp, Zap, Users, AlertTriangle,
  CheckCircle, Loader2, Upload, Shield, Brain, Smile, Layers,
  Download, Radio, GitBranch, Wifi, FileText
} from 'lucide-react'
...
function LoadingState({ jobStatus, progressEvents }) {
  const currentStage = progressEvents?.length > 0
    ...
    <div className="flex flex-col items-center justify-center min-h-screen gap-8 px-6"
      ...
      <div className="relative">
        ...
        <motion.div
          ...
          style={{ border: '2px solid #E5E7EB', borderBottom: '2px solid #28C76F' }}
        />
      ...
      <div className="text-center">
        <div className="font-bold text-2xl mb-2" style={{ color: '#1E293B', fontFamily: 'DM Sans, sans-serif' }}>
          {jobStatus === 'queued' ? 'Queueing 9-Agent Pipeline…' : 'Running Agentic Analysis…'}
        </div>
      ...
      <div className="flex flex-wrap gap-2 justify-center max-w-2xl">
        {PIPELINE_STAGES.map((stage, i) => {
          ...
          return (
            <motion.div
              key={stage.key}
              animate={isActive ? { opacity: [0.6, 1, 0.6] } : { opacity: isDone ? 1 : 0.35 }}
...
function EmptyState() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-6 text-center px-8"
      ...
      <div>
        <h2 className="font-bold text-2xl mb-2" style={{ color: '#1E293B', fontFamily: 'DM Sans, sans-serif' }}>
          No Analysis Yet
        </h2>
...
export default function DashboardPage() {
  const { report, jobStatus, activeTab, setActiveTab, progressEvents, downloadPDF, currentJob } = useReviewStore()
  const [liveFeedOpen, setLiveFeedOpen] = useState(false)
  const isLoading = jobStatus === 'running' || jobStatus === 'queued'

  if (isLoading) return <LoadingState jobStatus={jobStatus} progressEvents={progressEvents} />
  ...
  return (
    <div className="min-h-screen" style={{ background: '#F8FAFC' }}>
      {/* ── Sticky Header ── */}
      <div className="sticky top-0 z-20 px-6 pt-4 pb-0"
        style={{
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              <span className="text-xs" style={{ color: '#94A3B8' }}>
                ID: <span className="font-mono font-semibold" style={{ color: '#FF7A00' }}>{report.report_id}</span>
              </span>
              <span style={{ color: '#CBD5E1' }}>·</span>
              <span className="text-xs font-semibold" style={{ color: '#28C76F' }}>
              ...
              {report.trend_alerts?.some(a => a.severity === 'critical') && (
                <>
                  ...
                  >
                    ...
                    {report.trend_alerts.filter(a => a.severity === 'critical').length} critical
                  </motion.span>
                </>
              )}
            </div>
          ...
          <div className="flex items-center gap-3">
            {/* Quick stats */}
            {[
              { val: report.total_reviews,              label: 'Total',  color: '#2563EB', bg: '#DBEAFE' },
              { val: report.clean_reviews,              label: 'Clean',  color: '#28C76F', bg: '#DCFCE7' },
              { val: report.bot_suspected_count,        label: 'Bots',   color: '#EA5455', bg: '#FEE2E2' },
              { val: report.trend_alerts?.length || 0,  label: 'Alerts', color: '#D97706', bg: '#FEF3C7' },
            ].map(({ val, label, color, bg }) => (
              <div key={label}
                className="text-center px-3 py-2 rounded-xl hidden lg:block"
                style={{ background: bg, border: `1px solid ${color}30` }}>
                <div className="font-bold text-lg leading-none" style={{ color, fontFamily: 'DM Sans, sans-serif' }}>{val}</div>
            ...
            <motion.button
              ...
              style={{
                ...
                boxShadow: liveFeedOpen ? '0 0 0 3px rgba(255,122,0,0.1)' : 'none',
              }}
          ...
          {TABS.map(({ id, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                <Icon size={13} />
                {label}
                {id === 'trends' && report.trend_alerts?.some(a => a.severity === 'critical') && (
                ...
                {id === 'emoji' && (report.emoji_analysis?.total_emojis_found || 0) > 0 && (
                  <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 bg-orange-400" />
                )}
                {id === 'cross_compare' && report.cross_product_comparison?.categories?.length > 1 && (
      ...
      <AnimatePresence>
        {liveFeedOpen && (
          <LiveFeedTicker isOpen={liveFeedOpen} onToggle={() => setLiveFeedOpen(false)} />
        )}
      </AnimatePresence>
