...
import { BarChart3, Upload, Brain, TrendingUp, FileText, Activity, Zap } from 'lucide-react'
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: '#F8FAFC' }}>
      ...
      <motion.aside
        ...
        style={{ borderColor: '#E5E7EB', background: '#FFFFFF', boxShadow: '2px 0 12px rgba(0,0,0,0.04)' }}
      >
        {/* Logo */}
        <div className="p-6 border-b" style={{ borderColor: '#E5E7EB' }}>
          <button onClick={() => navigate('/')} className="flex items-center gap-3 group">
            ...
            <div>
              <div className="font-bold text-lg leading-none" style={{ color: '#1E293B', fontFamily: 'DM Sans, sans-serif' }}>
                ReviewIQ
              </div>
              <div className="text-xs mt-0.5 font-medium" style={{ color: '#94A3B8' }}>Intelligence Platform</div>
            </div>
          </button>
        </div>
          ...
          {NAV_ITEMS.map(({ to, icon: Icon, label }) => (
            <NavLink key={to} to={to}>
              {({ isActive }) => (
                ...
                >
                  <Icon size={16} style={{ color: isActive ? '#FF7A00' : '#94A3B8' }} />
                  {label}
                  {isActive && (
                    <motion.div
                      layoutId="active-nav"
        ...
        <div className="p-4 mt-auto border-t" style={{ borderColor: '#E5E7EB' }}>
          <div className="text-xs font-bold uppercase tracking-wider mb-3" style={{ color: '#CBD5E1' }}>
            Agent Pipeline
          </div>
          ...
          {jobStatus === 'running' && (
            <motion.div
              ...
              style={{ background: '#FFF3E8', border: '1px solid #FED7AA', color: '#FF7A00' }}
            >
              ⚡ Processing…
            </motion.div>
          )}
